					...
					<tbody>
						<tr>
							...
							<td class="query_btns" colspan="6">
                                       <button type="button" id="btn_searchonfirmList"  class="btn btn-small btn-primary"><i class="icon-search icon-white"></i> 搜索 </button>
                                        &nbsp;&nbsp;
...
jQuery(document).ready(function() {
 ...
 //初始化列表
  $("#btn_searchonfirmList").click(function(){
	  searchConfirmList();
  });
  searchConfirmList();	
});
function searchConfirmList(){
	var url="${ctx}/policyApproval/confirmSeachList";
	var theForm = document.forms[0];
	document.forms[0].action=url;
    theForm.target="approvalPolicyInfoList";
    theForm.submit();
}
function searchPageConfirmList(url){
	var theForm = document.forms[0];
	document.forms[0].action=url;
    theForm.target="approvalPolicyInfoList";
    theForm.submit();
}
function open_dialog(title,src,v_width,v_height){

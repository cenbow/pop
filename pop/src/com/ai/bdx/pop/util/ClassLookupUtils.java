...
public class ClassLookupUtils {
	...
	public static Collection<Class<?>> getClasses(ClassFileFilter filter) {
		...
		loaders.addAll(getClassLoaders(ClassLookupUtils.class.getClassLoader()));
	...
	public static Set<Class<?>> getClasses(String packageName, boolean recursive, ClassFileFilter filter) {
		...
		for (URL url : urls) {
			...
			if ("file".equals(protocol)) {
				String fileUrl = "";
				try {
					fileUrl = java.net.URLDecoder.decode(url.getFile(), "utf-8");
				} catch (UnsupportedEncodingException e1) {
				}
				lookupClassesInFileSystem(packageName, new File(fileUrl), recursive, loader, filter, klasses);
	 ...
	 */
	private static void lookupClassesInFileSystem(String packageName, File packagePath, final boolean recursive,
			ClassLoader loader, ClassFileFilter filter, Set<Class<?>> klasses) {
	 ...
	 * @param klasses       返回找到的 class
	 */
	private static void lookupClassesInJar(String packageName, URL jarUrl, boolean recursive, ClassLoader loader,
			ClassFileFilter filter, Set<Class<?>> klasses) {
		...
		try {
			...
			String jarFileUrl = java.net.URLDecoder.decode(jarUrl.getFile(), "utf-8");

			if ("jar".equals(jarUrl.getProtocol())) {
				jar = ((JarURLConnection) jarUrl.openConnection()).getJarFile();
			} else {
				jar = new JarFile(jarFileUrl);
			}

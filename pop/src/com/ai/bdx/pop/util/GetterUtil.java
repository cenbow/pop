...
public class GetterUtil {
	...
	public static boolean get(String strValue, boolean defaultValue) {
	...
	public static double get(String value, double defaultValue) {
		try {
			return Double.parseDouble(innerTrim(value));
		} catch (Exception e) {
		}
	...
	public static float get(String value, float defaultValue) {
		try {
			return Float.parseFloat(innerTrim(value));
		} catch (Exception e) {
		}
	...
	public static int get(String value, int defaultValue) {
		try {
			return Integer.parseInt(innerTrim(value));
		} catch (Exception e) {
		}
	...
	public static long get(String value, long defaultValue) {
		try {
			return Long.parseLong(innerTrim(value));
		} catch (Exception e) {
		}
	...
	//		try {
	//			return Short.parseShort(innerTrim(value));
	//		} catch (Exception e) {
	//		}
	//
	//		return defaultValue;
	...
	public static String get(String strValue, String defaultValue) {
		if (strValue != null && strValue.trim().length() > 0) {
			String value = strValue.trim();
			value = StringUtil.replaceAll(value, StringPool.RETURN_NEW_LINE, StringPool.NEW_LINE);
	...
	private static String innerTrim(String strValue) {
		String value = null;
		if (strValue != null) {
			value = strValue.trim();
			...
			for (int i = 0; i < charArray.length; i++) {
				if ((Character.isDigit(charArray[i])) || ((charArray[i] == CharPool.DASH) && (i == 0)) || (charArray[i] == CharPool.PERIOD) || (charArray[i] == CharPool.UPPER_CASE_E)

...
public class JedisClusterFactory {
	...
	/**JedisCluster单例*/
	private static JedisCluster jedisCluster2;
	/**集群节点 */
	private static String clusterNodes2;
	 ...
	 */
	static{
		clusterNodes2 = ConfigDataUtil.getRedisPropVal("redis.cluster.nodes");
	}
	 ...
	 * @throws Exception 
	 */
	private static JedisCluster genJedisCluster()throws Exception{
		JedisCluster jc = null;
		try {
			System.out.println("开始获得cluster实例.......");
			System.out.println("clusterNodes2="+clusterNodes2);
			Set<HostAndPort> haps = parseHostAndPort(clusterNodes2);
			if(haps != null){
				GenericObjectPoolConfig config = new GenericObjectPoolConfig();
				System.out.println("开始配置GenericObjectPoolConfig连接池.......");
				/*
				...
				*/
				...
				jc = new JedisCluster(haps, Integer.parseInt(Configure.getInstance().getProperty("redis.cluster.pool.timeout")), 
						Integer.parseInt(Configure.getInstance().getProperty("redis.cluster.pool.maxRedirections")), config);
				log.debug(">>>>>> redis cluster2连接节点 =" + jc.getClusterNodes());
				System.out.println(">>>>>> redis cluster2连接节点 =" + jc.getClusterNodes());
			}
		}catch(JedisException e){
			e.printStackTrace();
	...
	private static Set<HostAndPort> parseHostAndPort(String clusterNodes) {
        try {
            ...
            for (String ipPort : ipPorts) {
                ...
                System.out.println("HostAndPort:"+hap.toString());
                haps.add(hap);
            }
            return haps;
        } catch (Exception ex) {

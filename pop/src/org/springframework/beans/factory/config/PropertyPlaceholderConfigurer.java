...
public class PropertyPlaceholderConfigurer extends PropertyResourceConfigurer
		...
		while (startIndex != -1) {
			...
			if (endIndex != -1) {
				...
				if (propVal != null) {
					...
					//--add by lyz 2013/11/19 增加多省市注入判断,如果有此省市实现类类则注入,没有则注入默认---------
			 		if(strVal.indexOf("com.ai.bdx.frame.approval")!=-1 && strVal.indexOf("${PROVINCE}")!=-1){
						//判断是否截取的是PROVINCE
			 			if("${PROVINCE}".equals(buf.substring(startIndex, endIndex + this.placeholderSuffix.length()))){
							//是,则判断当前省市有无此service实现类,没有则替换成默认实现类
			 			     isHaveThisClass(buf,startIndex, endIndex + this.placeholderSuffix.length(), propVal);
			 			}else{
			 				//不是,走原来spring逻辑,直接按占位符替换
	...
	/**
	 * 判断当前省市,有无对应实现类,有就调用各自的,没有就调用默认的
	 */
	public void isHaveThisClass(StringBuffer className,int begin,int end,String replaceProvince){
		  
		  StringBuffer classNameTmp  = new StringBuffer(className);
     	  classNameTmp.replace(begin, end, replaceProvince);
	    try {
			   Class.forName(classNameTmp.toString());
			   logger.info("当前省市："+replaceProvince+",已经实现serviceImpl代码,不替换defaults代码,路径:"+classNameTmp.toString());
 			   className.replace(begin, end, replaceProvince);
		 } catch (ClassNotFoundException e) {
			   StringBuffer tmp = new StringBuffer(className);
			   className.replace(begin, end, "defaults");
			   logger.info("当前省市"+replaceProvince+",没有对应serviceImpl,将当前class:"+tmp.toString()+"替换为:-->"+className.toString());
		 }
		
//		  File fApp = new File(this.getClass().getResource("/").getPath());
//		  String appPath =  fApp.getPath().replace("%20", " ");
//		  StringBuffer classNameTmp  = new StringBuffer(className);
//		  classNameTmp.replace(begin, end, replaceProvince);
//		  String provinceClassName = classNameTmp.toString();
//		  String provinceClassPath=provinceClassName.replace(".", "\\");
//		  String completeProvinceClassPath = appPath+"\\"+provinceClassPath+".class";
//		  completeProvinceClassPath = completeProvinceClassPath.replace("\\", "/");
//		  logger.info("当前类路径:"+completeProvinceClassPath);
//		  File homePageFile = new File(completeProvinceClassPath);
//		  if(homePageFile.exists()){
//			  logger.info("当前省市："+replaceProvince+",已经实现serviceImpl代码,不替换defaults代码,路径:"+classNameTmp.toString());
//			  className.replace(begin, end, replaceProvince);
//		  }else{
//			   //用defaults 替换 省市代码,防止spring注入时 class not found
//			   StringBuffer tmp = new StringBuffer(className);
//			   className.replace(begin, end, "defaults");
//			   logger.info("当前省市"+replaceProvince+",没有对应serviceImpl,将当前class:"+tmp.toString()+"替换为:-->"+className.toString());
//			   

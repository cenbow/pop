...
public class TransferClientHandler extends IoHandlerAdapter {
	
	private static Logger log = LogManager.getLogger();
	
	public void sessionClosed(IoSession session) throws Exception {
		log.error("-------pop会话关闭!-------");
	//	HandlerData2EsperObserver.sessionsMapping.remove(session.getAttribute("flag"));
	//	session.close(true);
	}
	...
	public void exceptionCaught(IoSession session, Throwable cause)
			throws Exception {
		log.error("-------pop连接出现异常：-----------" + session.getRemoteAddress(),cause);
	//	HandlerData2EsperObserver.sessionsMapping.remove(session.getAttribute("flag"));
	//	session.close(true);
	}
	...
	public void sessionOpened(IoSession session) throws Exception {
		log.info("Adapter连接Pop成功：" + session.getRemoteAddress());
	}
}
